...
class TaskDetailsPage extends StatefulWidget {
  const TaskDetailsPage({Key? key, required this.id, required this.project_id})
      : super(key: key);
  final id;
  final project_id;
...
class _TaskDetailsPageState extends State<TaskDetailsPage> {
  ...
  void _showTask(int id) async {
    if (id != null) {
      ...
      setState(() {
        ...
        people_data = task_data['tasks_assigned_peoples']
            .replaceAll('[', '')
            .replaceAll(']', '');
  ...
  Future<void> _navigateforResult(BuildContext context) async {
    final result = await Navigator.push(context,
        MaterialPageRoute(builder: (context) => AddSubTaskPage(id: widget.id)));
    if (!mounted) return;
    initState();
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: <Widget>[
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            child: Text('$task_title'.toUpperCase(),
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            padding: EdgeInsets.all(8),
                            child:
                                Text('$task_description', style: subtitleStyle),
                          ),
                        ],
                      ),
                      SizedBox(
                      ...
                      Row(
                        children: [
                          Container(
                            width: wp(90, context),
                            padding: EdgeInsets.all(10),
                            child: Text('$task_enddate', style: subtitleStyle),
                          ),
                        ],
                      ),
                      SizedBox(
              ...
              Container(
                child: _listSubTasks.isNotEmpty
                    ? ListView.builder(
                        primary: false,
                        scrollDirection: Axis.vertical,
                        shrinkWrap: true,
                        ...
                        itemBuilder: (context, index) => Container(
                          ...
                          child: Slidable(
                            enabled:
                                _listSubTasks[index]['is_enable_subtasks'] == 1
                                    ? false
                                    : true,
                                                ...
                                                child: Padding(
                                                  padding:
                                                      const EdgeInsets.only(
                                                          right: 8),
                                                  child: Text(
                                                    '${(_listSubTasks[index]['subtasks_progress'] / _listSubTasks.length * 100).toInt()}%',
                                                    style: TextStyle(
                                                      fontFamily: 'lato',
                                                      color: Colors.white,
                                                      fontWeight:
